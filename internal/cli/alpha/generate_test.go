...
var _ = Describe("NewScaffoldCommand", func() {
	When("NewScaffoldCommand", func() {
		It("Testing the NewScaffoldCommand", func() {
			...
			Expect(cmd.Long).To(ContainSubstring("latest installed Kubebuilder version and plugins"))
			Expect(cmd.Long).NotTo(ContainSubstring("as update your project"))
			Expect(cmd.Example).To(ContainSubstring("**WARNING**"))
		})
	})
})
