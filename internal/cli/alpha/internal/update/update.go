...
func (opts *Update) Update() error {
	...
	if opts.Push {
		...
		pushCmd := helpers.GitCmd(opts.GitConfig, "push", "-u", "origin", out)
		// Capture both stdout and stderr to show the actual git error
		output, err := pushCmd.CombinedOutput()
		if err != nil {
			log.Error("Git push failed", "branch", out, "error", err, "output", string(output))
			return fmt.Errorf("failed to push %s: %w\nGit output: %s", out, err, string(output))
		}
		log.Info("Successfully pushed update branch", "branch", out)
	}
...
func preserveBoilerplate() []byte {
	...
	if err != nil {
		// Boilerplate file doesn't exist or can't be read - this is not an error
		return nil
	}
...
func restoreBoilerplate(content []byte) error {
	...
	boilerplatePath := filepath.Join("hack", "boilerplate.go.txt")
	if err := os.MkdirAll("hack", 0755); err != nil {
		return fmt.Errorf("failed to create hack directory: %w", err)
	}

	if err := os.WriteFile(boilerplatePath, content, 0644); err != nil {
