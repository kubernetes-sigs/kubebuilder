...
For example, you should refrain from moving the scaffolded files, as doing so will make it difficult to upgrade
...
may need to use the manual migration process, as automated migration tools might not work properly.
...
configuration. You can then manually compare and merge your custom code.

This approach is useful for projects that heavily customize the scaffold or when other migration methods aren't available.
...
This method is typically used for projects created with older Kubebuilder versions before the automated
update features were introduced, or for projects that have heavily customized their scaffold.
