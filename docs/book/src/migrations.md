...
Upgrading your project scaffold to adopt the latest changes in Kubebuilder m
ay involve migrating to a new plugin version (e.g., `go.kubebuilder.io/v3` → `go.kubebuilder.io/v4`)
...
All inputs used by Kubebuilder are tracked in the `PROJECT` file.
If you use the CLI to generate your scaffolds, this file will record the project's configuration and metadata.
It plays a central role during automated migration processes by providing the required
...
```bash
kubebuilder alpha generate
kubebuilder alpha generate --input-dir="./path/to/project" --output-dir="./new-project"
```
...
kubebuilder alpha update --from-version=v4.6.0 --to-version=v4.7.0
kubebuilder alpha update --from-version=v4.6.0 --to-version=v4.7.0 --from-branch=main
```
