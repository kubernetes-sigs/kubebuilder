...
For charts without conversion webhooks, install without webhook resources:

```bash
$ helm install my-release ./dist/chart --set webhook.enabled=false --set certManager.enabled=false
```

Charts with CR version conversion reject `webhook.enabled=false` while `crd.enabled=true`. The API server needs the webhook `Service` to convert custom resources.

<aside class="warning" role="note">
<p class="note-title"><code>webhook.enabled</code> does not stop the webhook server</p>
...
This setting works only when `cmd/main.go` retains the webhook guard generated by Kubebuilder and no other application code adds the webhook server to the manager. If you changed that setup, disable webhook registration and server startup in your application instead.
