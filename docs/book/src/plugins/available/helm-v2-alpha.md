...
For charts without conversion webhooks, disable the manager and optional components:
...
Charts with conversion CRDs reject `webhook.enabled=false`. The API server needs the webhook Service to convert custom resources.
...
Set `webhook.port` to change the manager argument, container port, Service target port, and NetworkPolicy. The Service port stays `443`.
...
When the kustomize output includes `NetworkPolicy` resources, the plugin converts and enables them. Otherwise, it generates default policies for metrics and detected webhooks.
...
The metrics policy is gated on both `networkPolicy.enabled` and `metrics.enabled`; the webhook policy is gated on both `networkPolicy.enabled` and `webhook.enabled`.

The metrics policy only allows traffic from namespaces labeled `metrics: enabled`, so label the namespaces that must scrape the metrics:

```bash
$ kubectl label namespace <namespace> metrics=enabled
```

The webhook policy allows all sources to reach the webhook port because API server traffic cannot be selected reliably. Use `namespaceSelector` in admission webhook configurations to limit which namespaces trigger admission.
