...
[controller-tools][controller-tools], [kustomize][kustomize-release]
and [kb-releases][kb-releases] release notes.

## Common changes

- `go/v4-alpha` projects use Kustomize v4x (instead of v3x)
- note that some manifests under `config/` directory changes in order to no longer use the deprecated Kustomize features
such as env vars. 
- the go/v4-alpha`plugin scaffolds the api/controllers under the directory pkg as requested by the community
...
So you want to upgrade your scaffolding to use the latest and greatest features then, follow up the following guide 
