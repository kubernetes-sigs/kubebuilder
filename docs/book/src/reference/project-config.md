...
| `resources.domain`                  | The domain of the resource. It comes from the `--domain` flag you used to initialize the project, or from the `--domain` flag of `create api`/`create webhook` when it names a resource under a different domain, such as an [External Type][external-type]. Together with the group, version and kind it identifies the resource, so a later command must pass `--domain` when several resources share the same group, version and kind. The deprecated `--external-api-domain` flag is an alias of `--domain`. |
