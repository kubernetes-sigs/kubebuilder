...
| `resources.domain`                  | The domain of the resource that you provided by the `--domain` flag when you initialized the project or via the flag `--external-api-domain` when you used it to scaffold controllers for an [External Type][external-type]. The domain also tells the resources apart when several of them share the group, version and kind, see [External Type][external-type].                                                   |
