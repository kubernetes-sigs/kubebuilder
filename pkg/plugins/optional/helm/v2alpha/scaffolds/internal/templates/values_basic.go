...
func (f *HelmValuesBasic) generateBasicValues() string {
	...
	// Header
	buf.WriteString(fmt.Sprintf(`# Default values for %s.
# This chart is generated from your kustomize manifests.

`, f.ProjectName))

	// Namespace configuration
	buf.WriteString(`# Control namespace creation and naming
namespace:
...
`)
	...
	buf.WriteString(`# Controller metrics endpoint is disabled by default (--metrics-bind-address=0).
...
func (f *HelmValuesBasic) addDeploymentConfig(buf *bytes.Buffer) {
	...
	if env, exists := f.DeploymentConfig["env"]; exists && env != nil {
		buf.WriteString("  # Environment variables from your deployment\n")
		buf.WriteString("  env:\n")
