...
var _ = Describe("ChartScaffolder", func() {
	Describe("PrepareTemplates", func() {
		It("should add the generic metrics NetworkPolicy when it is missing", func() {
			...
			Expect(err).NotTo(HaveOccurred())
			Expect(string(values)).To(SatisfyAny(
				ContainSubstring("networkPolicy:\n  enable: false"),
				ContainSubstring("networkPolicy:\n  enabled: false"),
			))
		})
		...
		It("should place all NetworkPolicies from kustomize output in the network-policy directory", func() {
			...
			Expect(err).NotTo(HaveOccurred())
			Expect(string(metricsPolicy)).To(SatisfyAny(
				ContainSubstring("{{- if .Values.networkPolicy.enable }}"),
				ContainSubstring("{{- if .Values.networkPolicy.enabled }}"),
			))
			Expect(string(metricsPolicy)).To(ContainSubstring("metrics: enabled"))
			...
			Expect(err).NotTo(HaveOccurred())
			Expect(string(dnsPolicy)).To(SatisfyAny(
				ContainSubstring("{{- if .Values.networkPolicy.enable }}"),
				ContainSubstring("{{- if .Values.networkPolicy.enabled }}"),
			))
			...
			Expect(err).NotTo(HaveOccurred())
			Expect(string(webhookPolicy)).To(SatisfyAny(
