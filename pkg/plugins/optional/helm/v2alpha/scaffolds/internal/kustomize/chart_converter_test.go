...
var _ = Describe("ChartConverter", func() {
	...
	Context("WriteChartFiles", func() {
		It("should write chart files to filesystem", func() {
			...
			exists, err := afero.Exists(fs.FS, "dist/chart/templates/namespace.yaml")
			Expect(err).NotTo(HaveOccurred())
		...
		It("should handle empty resource groups", func() {
			emptyResources := &ParsedResources{}
			emptyConverter := NewChartConverter(emptyResources, "test", "dist")

			err := emptyConverter.WriteChartFiles(fs)
			Expect(err).NotTo(HaveOccurred())

			// With empty resources, no files should be created
			exists, err := afero.Exists(fs.FS, "dist/chart/templates/namespace.yaml")
			Expect(err).NotTo(HaveOccurred())
			Expect(exists).To(BeFalse())

			// Check templates directory might not even exist
			_, err = afero.Exists(fs.FS, "dist/chart/templates")
			Expect(err).NotTo(HaveOccurred())
