...
// IsScaffoldedPolicyName reports whether name is the given scaffolded NetworkPolicy name,
// bare or prefixed with the chart name. The leading '-' keeps names such as
// 'disallow-metrics-traffic' from matching.
func IsScaffoldedPolicyName(name, policy string) bool {
	return name == policy || strings.HasSuffix(name, "-"+policy)
}
...
func TemplatePorts(yamlContent string, resource *unstructured.Unstructured) string {
	...
	isWebhook := (resourceKind == common.KindService && strings.HasSuffix(resourceName, "-webhook-service")) ||
		(resourceKind == common.KindNetworkPolicy && IsScaffoldedPolicyName(resourceName, "allow-webhook-traffic"))
	...
	isMetrics := (resourceKind == common.KindService &&
		...
		(resourceKind == common.KindNetworkPolicy && IsScaffoldedPolicyName(resourceName, "allow-metrics-traffic"))
