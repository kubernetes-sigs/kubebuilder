...
func templateEnvironmentVariables(yamlContent string) string {
	...
	for i := range lines {
		...
		block := make([]string, 0, 26)
		block = append(block,
			indentStr+"env:",
			indentStr+hasEnv,
			childIndent+`{{- /* Collect override names. */ -}}`,
			childIndent+`{{- $envOverrides := dict }}`,
			childIndent+`{{- $overrideNames := list }}`,
			childIndent+`{{- if kindIs "map" .Values.manager.envOverrides }}`,
			childIndent+`{{- $envOverrides = deepCopy .Values.manager.envOverrides }}`,
			childIndent+`{{- range $name, $_ := $envOverrides }}`,
			childIndent+`{{- $overrideNames = append $overrideNames $name }}`,
			childIndent+`{{- end }}`,
			childIndent+`{{- end }}`,
			childIndent+`{{- /* Append or replace existing variables in order. */ -}}`,
			childIndent+`{{- $env := list }}`,
			childIndent+`{{- range .Values.manager.env }}`,
			childIndent+`{{- if not (has .name $overrideNames) }}`,
			childIndent+`{{- $env = append $env . }}`,
			childIndent+`{{- else }}`,
			childIndent+`{{- $env = append $env (dict "name" .name "value" (get $envOverrides .name | toString)) }}`,
			childIndent+`{{- $_ := unset $envOverrides .name -}}`,
			childIndent+`{{- end }}`,
			childIndent+`{{- end }}`,
			childIndent+`{{- /* Fail if an override does not match an existing variable. */ -}}`,
			childIndent+`{{- range $envOverrides }}`,
			childIndent+`{{- fail (printf "manager.envOverrides contains unknown environment variable(s): %s" `+
				`(keys $envOverrides | sortAlpha | join ", ")) }}`,
			childIndent+`{{- end }}`,
			childIndent+`{{- /* Render the merged environment list. */ -}}`,
			childIndent+`{{- range $env }}`,
			childIndent+`{{- if hasKey . "value" }}`,
			childIndent+`- name: {{ .name }}`,
			childIndent+`  value: {{ .value | quote }}`,
			childIndent+`{{- else }}`,
			childIndent+"{{- toYaml (list .) | nindent "+childIndentWidth+" }}",
			childIndent+`{{- end }}`,
			childIndent+`{{- end }}`,
			childIndent+`{{- else }}`,
			childIndent+"[]",
