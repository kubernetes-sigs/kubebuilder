...
metadata:
  name: test-project-memcached-viewer-rolebinding`
	...
	Context("namePrefix templating", func() {
		It("should template resource names with namePrefix", func() {
...
metadata:
  ...
  namespace: test-project-system`
			...
			// Should template with namePrefix
			Expect(result).To(ContainSubstring("name: {{ .Values.namePrefix }}-controller-manager"))
			Expect(result).NotTo(ContainSubstring("name: test-project-controller-manager"))
		})
		It("should template ServiceMonitor name with namePrefix", func() {
			serviceMonitorResource := &unstructured.Unstructured{}
			serviceMonitorResource.SetAPIVersion("monitoring.coreos.com/v1")
			serviceMonitorResource.SetKind("ServiceMonitor")
			serviceMonitorResource.SetName("test-project-controller-manager-metrics-monitor")
...
metadata:
  name: test-project-controller-manager-metrics-monitor`
			...
			// Should template with namePrefix
			Expect(result).To(ContainSubstring("name: {{ .Values.namePrefix }}-controller-manager-metrics-monitor"))
    ...
    metadata:
      labels:
        app.kubernetes.io/name: test-project`
			...
			count := strings.Count(result, "app.kubernetes.io/name: {{ include \"chart.name\" . }}")
...
spec:
  ...
  - port: 8080
	...
	Context("cert-manager resource name templating", func() {
		It("should template Certificate resource name with namePrefix", func() {
			cert := &unstructured.Unstructured{}
			cert.SetAPIVersion("cert-manager.io/v1")
			cert.SetKind("Certificate")
			cert.SetName("test-project-serving-cert")
...
metadata:
  ...
  namespace: test-project-system`
			...
			Expect(result).To(ContainSubstring("name: {{ .Values.namePrefix }}-serving-cert"))
			Expect(result).NotTo(ContainSubstring("name: test-project-serving-cert"))
		})

		It("should template Issuer resource name with namePrefix", func() {
			issuer := &unstructured.Unstructured{}
			issuer.SetAPIVersion("cert-manager.io/v1")
			issuer.SetKind("Issuer")
			issuer.SetName("test-project-selfsigned-issuer")
...
spec:
  selfSigned: {}`
			...
			Expect(result).To(ContainSubstring("name: {{ .Values.namePrefix }}-selfsigned-issuer"))
			Expect(result).NotTo(ContainSubstring("name: test-project-selfsigned-issuer"))
		})

		It("should template issuer reference in certificates with namePrefix", func() {
			cert := &unstructured.Unstructured{}
			cert.SetAPIVersion("cert-manager.io/v1")
			cert.SetKind("Certificate")
			cert.SetName("test-project-serving-cert")
  ...
  template:
    spec:
      serviceAccountName: test-project-controller-manager`
			...
			// All name fields should use namePrefix
			Expect(result).To(ContainSubstring("name: {{ .Values.namePrefix }}-controller-manager"))
			Expect(result).To(ContainSubstring("serviceAccountName: {{ .Values.namePrefix }}-controller-manager"))
			Expect(result).NotTo(ContainSubstring("name: test-project-controller-manager"))
		})
...
- kind: ServiceAccount
  name: test-project-controller-manager`
			...
			// All references should use namePrefix
			Expect(result).To(ContainSubstring("name: {{ .Values.namePrefix }}-leader-election-rolebinding"))
			Expect(result).To(ContainSubstring("name: {{ .Values.namePrefix }}-leader-election-role"))
			Expect(result).To(ContainSubstring("name: {{ .Values.namePrefix }}-controller-manager"))
		})
	})
})
