...
var _ = Describe("editSubcommand", func() {
	...
	Context("Scaffold with invalid provider", func() {
		It("should return an error for unknown provider", func() {
			Expect(subCmd.InjectConfig(cfg)).To(Succeed())
			subCmd.provider = "invalid-provider"
	...
	Context("Scaffold produces cmd/main.go", func() {
		var memFS machinery.Filesystem
		...
		It("should write cmd/main.go", func() {
			subCmd.provider = "kubeconfig"
			Expect(subCmd.Scaffold(memFS)).To(Succeed())
			exists, err := afero.Exists(memFS.FS, "cmd/main.go")
