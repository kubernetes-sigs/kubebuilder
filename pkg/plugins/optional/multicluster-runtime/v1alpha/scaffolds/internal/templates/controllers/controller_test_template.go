...
// ControllerTest scaffolds the controller unit test file, replacing the
// go/v4 version so that it uses mcreconcile.Request instead of reconcile.Request.
...
func (f *ControllerTest) SetTemplateDefaults() error {
	...
	f.TemplateBody = mcControllerTestTemplate
...
const mcControllerTestTemplate = `{{ .Boilerplate }}
