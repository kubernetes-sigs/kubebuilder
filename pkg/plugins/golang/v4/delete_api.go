...
// removeControllerSetupFlexible removes controller setup with flexible matching
// This handles plugin variations (e.g., deploy-image adds Recorder field)
func (p *deleteAPISubcommand) removeControllerSetupFlexible(mainPath string) error {
	...
	// Find and remove the controller setup block
	// Need to track brace nesting to find the correct closing brace
	startIdx := -1
	endIdx := -1
	braceDepth := 0
	for i, line := range lines {
		if strings.Contains(line, reconcilerType) && strings.Contains(line, "if err := (") {
			startIdx = i
			braceDepth = 0
		}
		if startIdx != -1 {
			...
			// When we're back to 0 braces, we've found the end
	...
	// Remove the block (including all lines from startIdx to endIdx)
