...
// cleanupExtraBlankLines removes extra blank lines introduced by go/format
// Specifically targets blank lines before scaffold markers
func cleanupExtraBlankLines(content string) string {
	...
	for i := 0; i < len(lines); i++ {
		...
		// Check if this is a blank line before a scaffold marker
		if strings.TrimSpace(line) == "" && i+1 < len(lines) {
			nextLine := strings.TrimSpace(lines[i+1])
			// Skip blank lines immediately before scaffold markers
			if strings.Contains(nextLine, "+kubebuilder:scaffold:") {
				// Check if the previous line is not also blank
...
// removeControllerSetupRegex removes controller setup using regex to handle plugin variations
// This handles cases like deploy-image plugin adding extra fields (e.g., Recorder)
func (p *deleteAPISubcommand) removeControllerSetupRegex(mainPath string) error {
	content, err := os.ReadFile(mainPath)
	if err != nil {
		return fmt.Errorf("failed to read main.go: %w", err)
	}

	// Build regex pattern that matches the controller setup block with optional extra fields
	...
	// Pattern matches the controller setup with:
	// - Required fields: Client, Scheme
	// - Optional fields (like Recorder) added by plugins
	// - Flexible whitespace
	pattern := fmt.Sprintf(
		`\tif err := \(&%s\{\s*\n`+
			`\s*Client:\s*mgr\.GetClient\(\),\s*\n`+
			`\s*Scheme:\s*mgr\.GetScheme\(\),\s*\n`+
			`(?:\s*\w+:.*,\s*\n)*`+ // Match optional extra fields
			`\s*\}\)\.SetupWithManager\(mgr\); err != nil \{\s*\n`+
			`\s*setupLog\.Error\(err, "Failed to create controller", "controller", "%s"\)\s*\n`+
			`\s*os\.Exit\(1\)\s*\n`+
			`\s*\}\s*\n`,
		reconcilerType,
		p.resource.Kind,
	...
	if !re.Match(content) {
		return fmt.Errorf("controller setup block not found")
	}

	// Replace the matched block with empty string
	newContent := re.ReplaceAll(content, []byte(""))

	if err := os.WriteFile(mainPath, newContent, 0o644); err != nil {
...
func (p *deleteAPISubcommand) removeControllerSetupFlexible(mainPath string) error {
	...
	// The formatted code preserves the structure correctly
	formattedCode := buf.String()

	// Clean up any duplicate blank lines that might have been introduced
	formattedCode = cleanupExtraBlankLines(formattedCode)
...
func (p *deleteAPISubcommand) removeControllerSetup(mainPath string) {
	...
	// Try exact match first (for standard go/v4 plugin)
	...
	// Try regex-based removal (handles plugin variations like deploy-image's Recorder field)
	if err := p.removeControllerSetupRegex(mainPath); err == nil {
		log.Info("Removed controller setup from main.go (regex match)", "controller", p.resource.Kind)
		return
	}

	// Fall back to AST-based removal as last resort
	if err := p.removeControllerSetupFlexible(mainPath); err != nil {
		...
		log.Info("Removed controller setup from main.go (AST match)", "controller", p.resource.Kind)
	}
}
