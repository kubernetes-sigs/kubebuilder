...
// prettyDiff creates a human-readable diff showing what changed between expected and actual.
// It highlights line-by-line differences and shows surrounding context.
func prettyDiff(expected, actual, filename string) string {
	if expected == actual {
		return ""
	}
	...
	// Track differences to show summary
	var differences []string
	contextLines := 3 // Show 3 lines of context around each difference
	...
	for i := 0; i < maxLines; i++ {
		...
		if expectedLine != actualLine {
			...
			diff.WriteString(fmt.Sprintf("--- Difference at line %d ---\n", i+1))

			// Context before
			for j := start; j < i && j < len(expectedLines) && j < len(actualLines); j++ {
				diff.WriteString(fmt.Sprintf("  %4d: %s\n", j+1, expectedLines[j]))
			}

			// The difference itself
			if i < len(expectedLines) {
				diff.WriteString(fmt.Sprintf("- %4d: %q\n", i+1, expectedLine))
			} else {
				diff.WriteString(fmt.Sprintf("- %4d: (missing line)\n", i+1))
			}

			if i < len(actualLines) {
				diff.WriteString(fmt.Sprintf("+ %4d: %q\n", i+1, actualLine))
			} else {
				diff.WriteString(fmt.Sprintf("+ %4d: (extra line removed)\n", i+1))
			}
			...
			for j := i + 1; j < end && j < len(expectedLines) && j < len(actualLines); j++ {
			...
			// Track for summary
			diffDesc := fmt.Sprintf("Line %d: expected %q, got %q",
				i+1,
				strings.TrimSpace(expectedLine),
				strings.TrimSpace(actualLine))
			differences = append(differences, diffDesc)
		}
	}

	if len(differences) > 0 {
		diff.WriteString("========== SUMMARY ==========\n")
		diff.WriteString(fmt.Sprintf("Found %d line(s) with differences:\n", len(differences)))
...
// expectFileContentEqual checks if two file contents match and provides detailed diff on failure
func expectFileContentEqual(actual, expected, filename string) {
	if actual != expected {
		diffOutput := prettyDiff(expected, actual, filename)
		Fail(fmt.Sprintf("File content mismatch for %s\n%s", filename, diffOutput))
	}
}
...
var _ = Describe("Delete Comprehensive Integration Tests", func() {
	...
	It("should verify all API deletion scenarios with exact state restoration", func() {
		...
		expectFileContentEqual(string(mainContentAfterDelete), string(mainContentBefore), "cmd/main.go")
	...
	It("should completely undo create webhook operation - before equals after delete", func() {
		...
		expectFileContentEqual(string(mainContentAfterDelete), string(mainContentBefore), "cmd/main.go (after webhook delete)")
	...
	It("should handle complete create-delete cycle with multiple APIs", func() {
		...
		expectFileContentEqual(string(mainContentFinal), string(mainContentBefore), "cmd/main.go (after deleting all APIs)")
		...
		expectFileContentEqual(string(mainContentAfter), string(mainContentBeforeMulti), "cmd/main.go (multigroup)")
	...
	It("should verify all webhook deletion scenarios with exact state restoration", func() {
		...
		expectFileContentEqual(string(crdContentAfterDelete), string(crdContentBefore), "config/crd/kustomization.yaml (after conversion webhook delete)")
		...
		expectFileContentEqual(string(crdAfterDelete), string(crdBaseline), "config/crd/kustomization.yaml (baseline with defaulting/validation)")
		...
		Expect(err).NotTo(HaveOccurred())
		expectFileContentEqual(string(mainAfterDelete), string(mainBaseline), "cmd/main.go (baseline with webhook code)")
	...
	It("should verify deploy-image plugin chain - baseline restored after deletion", func() {
		...
		mainAfterDelete, err := os.ReadFile(mainPath)
		Expect(err).NotTo(HaveOccurred())
		expectFileContentEqual(string(mainAfterDelete), string(mainBaseline), "cmd/main.go (after deploy-image delete)")
	...
	It("should handle API without controller and controller without API scenarios", func() {
		...
		mainAfterDelete, err := os.ReadFile(mainPath)
		Expect(err).NotTo(HaveOccurred())
		expectFileContentEqual(string(mainAfterDelete), string(mainBefore), "cmd/main.go (resource-only API delete)")
	...
	It("should handle webhooks on resource-only APIs correctly", func() {
		...
		mainAfterDelete, err := os.ReadFile(mainPath)
		Expect(err).NotTo(HaveOccurred())
		expectFileContentEqual(string(mainAfterDelete), string(mainBefore), "cmd/main.go (webhooks on resource-only API)")
	})
