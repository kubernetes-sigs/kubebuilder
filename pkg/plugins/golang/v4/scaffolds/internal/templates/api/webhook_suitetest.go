...
func (f *WebhookSuite) SetTemplateDefaults() error {
	if f.Path == "" {
		if f.MultiGroup {
			if f.Resource.Group != "" {
				f.Path = filepath.Join("pkg", "apis", "%[group]", "%[version]", "webhook_suite_test.go")
			} else {
				f.Path = filepath.Join("pkg", "apis", "%[version]", "webhook_suite_test.go")
			}
		} else {
			f.Path = filepath.Join("pkg", "api", "%[version]", "webhook_suite_test.go")
		}
	}
