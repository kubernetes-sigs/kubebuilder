...
func (f *Webhook) SetTemplateDefaults() error {
	if f.Path == "" {
		if f.MultiGroup {
			if f.Resource.Group != "" {
				f.Path = filepath.Join("pkg", "apis", "%[group]", "%[version]", "%[kind]_webhook.go")
			} else {
				f.Path = filepath.Join("pkg", "apis", "%[version]", "%[kind]_webhook.go")
			}
		} else {
			f.Path = filepath.Join("pkg", "api", "%[version]", "%[kind]_webhook.go")
		}
	}
