...
var _ = Describe("Server-Side Apply Plugin Scaffolding", func() {
	...
	It("should scaffold API with server-side-apply plugin correctly", func() {
		...
		By("verifying Makefile was updated with applyconfiguration for api/...")
		makefile, err := os.ReadFile(filepath.Join(kbc.Dir, "Makefile"))
		Expect(err).NotTo(HaveOccurred())
		Expect(string(makefile)).To(ContainSubstring("applyconfiguration"))
		Expect(string(makefile)).To(ContainSubstring("paths=\"./api/...\""))
	...
	It("should handle multiple APIs with mixed plugins", func() {
		...
		By("verifying Makefile has single applyconfiguration target for api/...")
		makefile, err := os.ReadFile(filepath.Join(kbc.Dir, "Makefile"))
		Expect(err).NotTo(HaveOccurred())
		Expect(string(makefile)).To(ContainSubstring("applyconfiguration"))
		Expect(string(makefile)).To(ContainSubstring("paths=\"./api/...\""))
