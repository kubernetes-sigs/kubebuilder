...
func (p *createAPISubcommand) Validate() error {
	...
	if p.options.Group == "" && p.config.Domain == "" {
	...
	if p.options.DoAPI {
		...
		if res := p.config.GetResource(p.options.GVK()); !p.force && res != nil && res.API != nil {
			return errors.New("API resource already exists")
		}

		// Check that the provided group can be added to the project
		if !p.config.MultiGroup && len(p.config.Resources) != 0 && !p.config.HasGroup(p.options.QualifiedGroup()) {
...
func (p *createAPISubcommand) GetScaffolder() (cmdutil.Scaffolder, error) {
	...
	return scaffolds.NewAPIScaffolder(p.config, string(bp), res, plugins), nil
}
