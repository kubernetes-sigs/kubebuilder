...
import (
	...
	"sigs.k8s.io/kubebuilder/v3/pkg/plugins/config-gen/v1alpha/scaffolds"
)
...
`
  ...
  %[1]s init --plugins config-gen/%[2]s --domain example.org 

  # Initialize a common project defining an specific project version
  %[1]s init --plugins config-gen/%[2]s --project-version 3
