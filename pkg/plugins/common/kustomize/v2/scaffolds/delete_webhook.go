...
func (s *deleteWebhookScaffolder) PostScaffold() error {
	...
	if s.deletedConversion {
		...
		// If not, recomment the configurations section (only conversion webhooks need it uncommented)
		if !s.hasAnyConversionWebhookRemaining() {
			s.recommentCRDKustomizeConfigurations()
		}
	}
...
// recommentCRDKustomizeConfigurations recommends the configurations section in config/crd/kustomization.yaml
// This should be called when the last conversion webhook is deleted (conversion webhooks need this uncommented)
func (s *deleteWebhookScaffolder) recommentCRDKustomizeConfigurations() {
...
`
	...
	if err := util.ReplaceInFile(crdKustomizePath, uncommented, commented); err != nil {
		...
		log.Info("Recommended configurations section in CRD kustomization (no conversion webhooks remain)",
...
func (s *deleteWebhookScaffolder) commentWebhookKustomizeConfiguration() {
  ...
  target:
	...
	// Note: CRD kustomization configurations section is handled separately in recommentCRDKustomizeConfigurations()
