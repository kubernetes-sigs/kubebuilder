...
func (cb *CertsBuilder) createCertTemplate() (*x509.Certificate, error) {
	...
	serviceNames := []string{cb.Name, cb.Name + ".svc", cb.Name + ".svc.cluster.local"}

	tmpl := x509.Certificate{
		SerialNumber:          serialNumber,
		Subject:               pkix.Name{Organization: []string{cb.Organization}},
...
func (cb *CertsBuilder) createCACertTemplate() (*x509.Certificate, error) {
	rootCert, err := cb.createCertTemplate()
	if err != nil {
		return nil, err
	}
...
func (cb *CertsBuilder) createServerCertTemplate() (*x509.Certificate, error) {
	serverCert, err := cb.createCertTemplate()
	if err != nil {
		return nil, err
	}
...
func (cb *CertsBuilder) createCA() (*rsa.PrivateKey, *x509.Certificate, []byte, error) {
	...
	rootCertTmpl, err := cb.createCACertTemplate()
	if err != nil {
...
func (cb *CertsBuilder) CreateCerts() (serverKey, serverCert, caCert []byte, err error) {
	// First create a CA certificate and private key
	caKey, caCertificate, caCertificatePEM, err := cb.createCA()
	if err != nil {
		return nil, nil, nil, err
	}
	...
	servCertTemplate, err := cb.createServerCertTemplate()
	if err != nil {
