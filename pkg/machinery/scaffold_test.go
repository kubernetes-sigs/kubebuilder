...
var _ = Describe("Scaffold", func() {
	Describe("NewScaffold", func() {
		It("should succeed for no option", func() {
			s, ok := NewScaffold(afero.NewMemMapFs()).(*scaffold)
			Expect(ok).To(BeTrue())
		...
		It("should succeed with directory permissions option", func() {
			...
			s, ok := NewScaffold(afero.NewMemMapFs(), WithDirectoryPermissions(dirPermissions)).(*scaffold)
			Expect(ok).To(BeTrue())
			Expect(s.fs).NotTo(BeNil())
		...
		It("should succeed with file permissions option", func() {
			...
			s, ok := NewScaffold(afero.NewMemMapFs(), WithFilePermissions(filePermissions)).(*scaffold)
			Expect(ok).To(BeTrue())
			Expect(s.fs).NotTo(BeNil())
			Expect(s.dirPerm).To(Equal(defaultDirectoryPermission))
		...
		It("should succeed with config option", func() {
			...
			s, ok := NewScaffold(afero.NewMemMapFs(), WithConfig(cfg)).(*scaffold)
			Expect(ok).To(BeTrue())
			Expect(s.fs).NotTo(BeNil())
			Expect(s.dirPerm).To(Equal(defaultDirectoryPermission))
			Expect(s.filePerm).To(Equal(defaultFilePermission))
		...
		It("should succeed with boilerplate option", func() {
			...
			s, ok := NewScaffold(afero.NewMemMapFs(), WithBoilerplate(boilerplate)).(*scaffold)
			Expect(ok).To(BeTrue())
			Expect(s.fs).NotTo(BeNil())
			Expect(s.dirPerm).To(Equal(defaultDirectoryPermission))
			Expect(s.filePerm).To(Equal(defaultFilePermission))
		...
		It("should succeed with resource option", func() {
			...
			s, ok := NewScaffold(afero.NewMemMapFs(), WithResource(res)).(*scaffold)
			Expect(ok).To(BeTrue())
			Expect(s.fs).NotTo(BeNil())
			Expect(s.dirPerm).To(Equal(defaultDirectoryPermission))
			Expect(s.filePerm).To(Equal(defaultFilePermission))
