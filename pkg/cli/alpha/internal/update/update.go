...
func (opts *Update) prepareAncestorBranch() error {
	...
	log.Infof("Successfully committed %s with clean scaffolding", opts.AncestorBranch)
	return nil
}
...
func binaryWithVersion(version string) (string, error) {
	...
	log.Infof("Downloading the release %s binary from: %s", version, url)
	...
	log.Infof("release binary version %s successfully downloaded to %s", version, binaryPath)
	return tempDir, nil
}
...
func (opts *Update) prepareOriginalBranch() error {
	...
	log.Infof("Successfully checked out %s", opts.OriginalBranch)
	...
	log.Infof("Successfully checked out content from %s onto %s branch", opts.FromBranch, opts.OriginalBranch)
	...
	log.Infof("Successfully committed changes in %s", opts.OriginalBranch)
	return nil
}
...
func (opts *Update) prepareUpgradeBranch() error {
	...
	log.Infof("Successfully checked out %s off %s", opts.UpgradeBranch, opts.AncestorBranch)
	...
	log.Infof("Successfully staged changes in %s", opts.UpgradeBranch)

	_ = exec.Command("git", "commit", "-m", "Clean scaffolding from version "+opts.ToVersion).Run()
	log.Infof("Successfully committed %s with clean scaffolding", opts.UpgradeBranch)
	return nil
}
...
func (opts *Update) mergeOriginalToUpgrade() error {
	...
	log.Infof("Checked out merge branch: %s", opts.MergeBranch)
	...
	log.Infof("Merging in original code (%s) into merge branch (%s)", opts.OriginalBranch, opts.MergeBranch)
