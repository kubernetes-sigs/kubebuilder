...
func (c *CLI) getInfoFromConfig(projectConfig config.Config) error {
	...
	if projectConfig.GetLayout() != "" {
		...
		for _, pluginKey := range pluginSet {
		...
		// Project version 2 did not store the layout field, but only the go.kubebuilder.io/v2 plugin was supported
		pluginSet = append(pluginSet, plugin.KeyFor(goPluginV2.Plugin{}))
	}

	c.pluginKeys = append(c.pluginKeys, pluginSet...)
