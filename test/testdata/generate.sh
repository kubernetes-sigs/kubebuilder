...
function scaffold_test_project {
  ...
  if [ $project == "project-v4" ] ; then
    ...
    $kb create api --group "cert-manager" --version v1 --kind Certificate --controller=true --resource=false --make=false --external-api-path=github.com/cert-manager/cert-manager/pkg/apis/certmanager/v1 --domain=io --external-api-module=github.com/cert-manager/cert-manager@v1.20.2
    # Webhook for External types
    $kb create webhook --group "cert-manager" --version v1 --kind Issuer --defaulting --external-api-path=github.com/cert-manager/cert-manager/pkg/apis/certmanager/v1 --domain=io --external-api-module=github.com/cert-manager/cert-manager@v1.20.2
