...
var _ = Describe("kubebuilder", func() {
	Context("plugin helm/v2-alpha", func() {
		...
		It("should work when chart name is renamed", func() {
			...
			By("renaming chart to validate labels use chart.name while resources use namePrefix")
			chartPath := filepath.Join(kbc.Dir, "dist", "chart")
			chartYamlPath := filepath.Join(chartPath, "Chart.yaml")
			customChartName := "renamed-chart"
			err = pluginutil.ReplaceInFile(chartYamlPath, "name: e2e-"+kbc.TestSuffix, "name: "+customChartName)
			Expect(err).NotTo(HaveOccurred())

			By("deploying with renamed chart - resources use namePrefix, labels use chart name")
			runHelm(kbc, true, true, false, "e2e-"+kbc.TestSuffix)
		})

		It("should work when namePrefix is overridden in values", func() {
			By("generating a full-featured project")
			generateHelmProject(kbc)
			...
			By("overriding namePrefix in values.yaml")
			valuesPath := filepath.Join(kbc.Dir, "dist", "chart", "values.yaml")
			err = pluginutil.ReplaceInFile(valuesPath, "namePrefix: e2e-"+kbc.TestSuffix, "namePrefix: custom-prefix")
			Expect(err).NotTo(HaveOccurred())

			By("deploying with custom namePrefix - all resources use the override")
			runHelm(kbc, true, true, false, "custom-prefix")
		})

		It("should respect custom kustomize namePrefix", func() {
			By("initializing project")
			initHelmProject(kbc)
			createAPI(kbc)

			By("customizing kustomize namePrefix before generating resources")
			kustomizePath := filepath.Join(kbc.Dir, "config", "default", "kustomization.yaml")
			oldPrefix := "namePrefix: e2e-" + kbc.TestSuffix + "-"
			newPrefix := "namePrefix: short-"
			err := pluginutil.ReplaceInFile(kustomizePath, oldPrefix, newPrefix)
			Expect(err).NotTo(HaveOccurred())

			By("installing Helm")
			Expect(kbc.InstallHelm()).To(Succeed())

			By("deploying - resources should use detected 'short' prefix from kustomize")
			runHelm(kbc, false, true, false, "short")
		})
	})
})
...
// If namePrefix is provided, it will be used for resource name lookups; otherwise defaults to "e2e-{suffix}"
func runHelm(kbc *utils.TestContext, hasWebhook, hasMetrics, hasNetworkPolicies bool, namePrefix string) {
	// Default to project name if no namePrefix provided
	if namePrefix == "" {
		namePrefix = "e2e-" + kbc.TestSuffix
	}

	// Update ServiceAccount name to match the actual namePrefix being used
	// This is critical because the Helm chart templates resource names using {{ .Values.namePrefix }}
	...
	if hasNetworkPolicies {
		if hasMetrics {
			...
			By("Ensuring the Allow Metrics Traffic NetworkPolicy exists", func() {
				...
				output, err = kbc.Kubectl.Get(
					true,
					"networkpolicy", fmt.Sprintf("%s-allow-metrics-traffic", namePrefix),
				)
		...
		if hasWebhook {
			...
			By("Ensuring the allow-webhook-traffic NetworkPolicy exists", func() {
				...
				output, err = kbc.Kubectl.Get(
					true,
					"networkpolicy", fmt.Sprintf("%s-allow-webhook-traffic", namePrefix),
				)
	...
	if hasMetrics {
		_ = getMetricsOutput(kbc, namePrefix, controllerPodName)
	} else {
		metricsShouldBeUnavailable(kbc, namePrefix)
	}
...
// namePrefix is the namePrefix value used for resource names
// controllerPodName is the name of the controller manager pod
func getMetricsOutput(kbc *utils.TestContext, namePrefix string, controllerPodName string) string {
	...
	checkServiceEndpoint := func(g Gomega) {
		...
		output, err = kbc.Kubectl.Get(
			true,
			"endpoints", fmt.Sprintf("%s-controller-manager-metrics-service", namePrefix),
	...
	cmdOpts := cmdOptsToCreateCurlPod(kbc, token, namePrefix)
...
func metricsShouldBeUnavailable(kbc *utils.TestContext, namePrefix string) {
	_, err := kbc.Kubectl.Command(
		"create", "clusterrolebinding", fmt.Sprintf("metrics-%s", kbc.TestSuffix),
		fmt.Sprintf("--clusterrole=%s-metrics-reader", namePrefix),
		fmt.Sprintf("--serviceaccount=%s:%s", kbc.Kubectl.Namespace, kbc.Kubectl.ServiceAccount))
...
func cmdOptsToCreateCurlPod(kbc *utils.TestContext, token, namePrefix string) []string {
	//nolint:lll
	cmdOpts := []string{
		"run", "curl",
		"--restart=Never",
