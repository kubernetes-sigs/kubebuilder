...
func validateIssuerNameTemplating(kbc *utils.TestContext, chartPath string) {
	...
	// Verify issuer name uses namePrefix template, not hardcoded project name
	Expect(contentStr).To(ContainSubstring(`name: {{ .Values.namePrefix }}-selfsigned-issuer`),
		"Issuer name should use namePrefix template")
...
func validateCertificateIssuerRefTemplating(_ *utils.TestContext, chartPath string) {
	...
	for _, file := range files {
		...
		if strings.Contains(contentStr, "kind: Certificate") {
			...
			// Verify certificate issuerRef uses namePrefix template
			Expect(contentStr).To(ContainSubstring(`name: {{ .Values.namePrefix }}-selfsigned-issuer`),
				"Certificate issuerRef should use namePrefix template in file "+file.Name())
		}
	}
	...
	if exists, _ := afero.DirExists(afero.NewOsFs(), webhookDir); exists {
		...
		for _, file := range files {
			...
			if strings.Contains(contentStr, "cert-manager.io/inject-ca-from") {
				// Verify cert-manager annotation uses namePrefix template
				Expect(contentStr).To(ContainSubstring(`{{ .Values.namePrefix }}-serving-cert`),
					"cert-manager.io/inject-ca-from annotation should use namePrefix template in "+file.Name())
